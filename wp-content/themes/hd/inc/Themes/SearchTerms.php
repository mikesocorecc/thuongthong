...
function add_url_filters() {
	...
	if ( \is_admin() || empty( SearchTerms::get() ) ) return;
...
function add_search_query_arg( $url ) {
	...
	if ( \in_the_loop() && ! \strpos( $url, 'hilite=' ) ) {
		$terms = array();
		foreach ( SearchTerms::get() as $term ) {
...
function enqueue_script() {
	...
	if ( $script_enqueued || empty( SearchTerms::get() ) ) return;
	...
	\wp_enqueue_script( "mark", get_template_directory_uri() . "/assets/js/plugins/mark.min.js", [ "jquery" ], '9.0.0', true );
	...
	$script = '/* Highlight Search Terms '.VERSION.' */' . \PHP_EOL;
